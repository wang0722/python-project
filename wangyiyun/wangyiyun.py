...
class MyApp(Tk):
    ...
    def setupUI(self):  #设置根窗口的UI
        ...
        fu=local()
        button = Button(self, text='路径选择', font=('微软雅黑', 15),command=fu.lujing)
    ...
    def write_lyric(self,song_name, lyric):
        if lyric == 0:
           ...
           if os.path.exists(path2):
              with open('log.txt', 'r', encoding='utf-8') as f:
                    self.line = f.readline()
              if os.path.exists(self.line+'\单曲下载\歌词\\{}.txt'.format(song_name)):
        ...
        if song_url is None:
            ...
            if os.path.exists(path2):
                ...
                if os.path.exists(self.line+'\单曲下载\歌曲\{}.mp3'.format(song_name)):
                   ...
                   urllib.request.urlretrieve(song_url, self.line+'\单曲下载\歌曲\\{}.mp3'.format(song_name))
            else:
    ...
    def write_lyric(self,song_name, lyric):
        ...
        if lyric == 0:
            ...
            if os.path.exists(path2):
                with open('log.txt', 'r', encoding='utf-8') as f:
                    self.line = f.readline()
                if os.path.exists(self.line + '\歌手下载\{}\歌词'.format(singer_name3) + '\{}.txt'.format(song_name)):
                    MyApp.text.insert(END, '歌词已存在：{}'.format(song_name))
                    MyApp.text.see(END)
                    MyApp.text.update()
                    pass
    ...
    def downloadsong(self,song_name, song_id):
        ...
        if song_url is None:
            ...
            if os.path.exists(path2):
                with open('log.txt', 'r', encoding='utf-8') as f:
                    self.line = f.readline()
                if os.path.exists(self.line + '\歌手下载\{}\歌曲'.format(singer_name3) + '\{}.mp3'.format(song_name)):
                    MyApp.text.insert(END, '歌曲已存在：{}'.format(song_name))
                    MyApp.text.see(END)
                    MyApp.text.update()
                    pass
                    ...
                    MyApp.text.see(END)
                    MyApp.text.update()
                    urllib.request.urlretrieve(song_url,self.line + '\歌手下载\{}\歌曲'.format(singer_name3) + '\{}.mp3'.format(
                                                   song_name))
            else:
    ...
    def write_lyric(self,song_name, lyric):
        ...
        if lyric == 0:
            ...
            if os.path.exists(path2):
                with open('log.txt', 'r', encoding='utf-8') as f:
                    self.line = f.readline()
                if os.path.exists(self.line+'\歌单下载\{}\歌词'.format(singer_name3)+'\{}.txt'.format(song_name)):
                   MyApp.text.insert(END, '歌词已存在：{}'.format(song_name))
                   MyApp.text.see(END)
                   MyApp.text.update()
                   pass
    ...
    def downloadsong(self,song_name, song_id):
        ...
        if song_url is None:
            ...
            if os.path.exists(path2):
                with open('log.txt', 'r', encoding='utf-8') as f:
                    self.line = f.readline()
                if os.path.exists(self.line+'\歌单下载\{}\歌曲'.format(singer_name3) + '\{}.mp3'.format(song_name)):
                   MyApp.text.insert(END,'歌曲已存在：{}'.format(song_name))
                   MyApp.text.see(END)
                   MyApp.text.update()
                   pass
                   ...
                   MyApp.text.see(END)
                   MyApp.text.update()
                   urllib.request.urlretrieve(song_url, self.line+'\歌单下载\{}\歌曲'.format(singer_name3)+'\{}.mp3'.format(song_name))
            else:
