...
class TreeNode(object):
    ...
    def __init__(self, n_weight=-1, n_pn="", n_parent=None):
        ...
        self.db = redis.StrictRedis(host='localhost',port=6379,db=0,password='')
    ...
    def insert(self, package_name, weight, sha256, permission_list, api_id_list):
        ...
        if target_package_name in self.children:
            ...
            # print self.children[target_package_name].insert(package_name, weight, sha256, permission_list, api_id_list)
...
class Tree(object):
    ...
    def __init__(self):
        ...
        self.feature = None
        self.db = redis.StrictRedis(host = 'localhost', port = 6379, db = 0, password = '')
        self.db_rep = redis.StrictRedis(host = 'localhost', port = 6379, db = 0, password = '')
        # self.db = redis.StrictRedis(host=DB_HOST, port=DB_PORT, db=DB_ID, password=DB_PSWD)
        # self.db_rep = redis.StrictRedis(host=DB_HOST, port=DB_PORT, db=DB_ID_REP, password=DB_PSWD)

    def insert(self, package_name, weight, sha256, permission_list, api_id_list):

        cc=self.root.insert(package_name, weight, sha256, permission_list, api_id_list)
        # return cc
    def brand(self, package_name, standard_package):
    ...
    def get_lib(self, res):
        ...
        aaa=self.pre_order_res(visit=self._get_lib, res=res)
        # print(aaa)
    @staticmethod
    ...
    def get_repackage_main(self, res, hex_sha256):
        ...
        aa=self.db_rep.zadd("apk_weight", mapping,hex_sha256)
        # print aa
